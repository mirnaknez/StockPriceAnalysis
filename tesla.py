...
def weekpredict(sentimenttimes, sentimentscores):
    ...
    if totalpolarity > 0:
        return "Prediction based on comments from the past week: TSLA stock price will rise."
    elif totalpolarity < 0:
        return "Prediction based on comments from the past week: TSLA stock price will fall."
    else:
        return "Prediction based on comments from the past week: TSLA stock price will not rise or fall."
...
def comparegraph(sentimenttimes, sentimentscores, stockdata):
    ...
    plt.xlabel("Time")
    plt.ylabel("Value")
    plt.title("Comparison of average sentiment and TSLA stock price")
    plt.legend()
    plt.grid()
    plt.show()
...
def main():
    comments1, timestamps1 = collectcomments("wallstreetbets", stock_keywords["Tesla"])
    comments2, timestamps2 = collectcomments("wallstreetbetsnew", stock_keywords["Tesla"])
    ...
    stockdata = getstockdata("TSLA", min(timestamps).strftime("%Y-%m-%d"), max(timestamps).strftime("%Y-%m-%d"))
    ...
    if majorityclass > 2:
        print("Model prediction for the past week: TSLA stock price will rise.")
    elif majorityclass < 2:
        print("Model prediction for the past week: TSLA stock price will fall.")
    else:
        print("Model prediction for the past week: TSLA stock price will not rise or fall.")
